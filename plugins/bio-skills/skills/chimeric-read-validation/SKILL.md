...
- Companion skills: `nfcore-module` (when the user wants to wrap chimeric-read validation as an nf-core module), `igv-reports` (visual inspection layer for spot-checks), `severus` rules in `analysis-gotchas` skill → `references/severus.md` (caller-side gotchas that explain why validation is needed).
