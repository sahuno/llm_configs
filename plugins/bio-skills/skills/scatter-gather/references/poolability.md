...
For DSS specifically: see `analysis-gotchas` skill → `references/dss.md` for the empirical-Bayes prior story, the `mclapply + detectCores()` silent-corruption incident, and post-run verification. Don't restate; just consult that file whenever DSS is in the loop.
...
|---|---|---|
| **DSS `DMLtest`** | Empirical-Bayes prior on all CpGs | **See `analysis-gotchas` skill → `references/dss.md`.** Per-chrom DSS ≠ same-chr slice of whole-genome. Real 2026-04-24 incident: 10–20× DMR inflation under SLURM memory pressure when `ncores` defaulted via `detectCores()`. |
