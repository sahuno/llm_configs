...
Per `mskcc-hpc` skill → `references/apptainer_vs_conda.md`: on this cluster, prefer SIF for short HTSlib-class
...
- **One condition per SLURM job.** `--exclusive`, single-node. Fresh node → cold page
  ...
  `mskcc-hpc` skill → `references/mskcc_partitions.md`.
- **GNU time, dynamically resolved.** Don't hardcode `/usr/bin/time` — it doesn't exist
  ...
  tool's binary. See `mskcc-hpc` skill → `references/gnu_time.md`.
...
- **slurm-mcp gotchas**: `submit_batch` injects `--mem 64G` on cmdline which conflicts
  with `#SBATCH --mem-per-cpu`; doesn't support `--array`. See `mskcc-hpc` skill → `references/slurm_mcp.md`.
