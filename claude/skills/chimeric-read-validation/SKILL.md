...
description: Read-level forensic validation of breakpoint-style SV calls — viral integrations (HTLV-1, HBV, HPV, EBV), gene fusions (BCR-ABL, IGH translocations), mobile element insertions (L1, Alu, SVA), inter-chromosomal translocations. Takes candidate breakpoints + tumor (± matched-normal) BAMs, extracts chimeric reads spanning each junction, and emits a per-call validation TSV with explicit verdicts (pass / needs_review / fail), turning "open IGV and look" into a deterministic, auditable step. Trigger PROACTIVELY whenever the user wants to validate / audit / QC / verify SV calls or breakpoints, asks "is this integration real?", "should I trust this fusion call?", "are these false positives?", wants per-call chimeric-read evidence, wants to check whether caller-PASS calls are supported at read level, run a per-call T/N contamination check, or types /chimeric-read-validation. Use it AFTER a caller (Severus, Manta, Delly, GRIDSS, Sniffles2, MELT, Arriba) has produced calls — this is the validation layer, not the caller.
---
